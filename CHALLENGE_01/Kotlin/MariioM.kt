fun main (){
    ...
    for(saved in wordList){
        var repeated = 0
        for(word in encryptedList){
            if(saved.equals(word, ignoreCase = true)){
                repeated ++
            }
        }
        repeatedWords.add(saved + repeated)
    }
    var decryptedText = repeatedWords.joinToString(separator="")
